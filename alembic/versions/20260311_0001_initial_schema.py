...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "contractor_registration_number", sa.String(length=64), nullable=False
        ),
    ...
    op.create_index(
        op.f("ix_bond_requests_broker_id"), "bond_requests", ["broker_id"], unique=False
    )
    op.create_index(
        op.f("ix_bond_requests_status"), "bond_requests", ["status"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["bond_request_id"], ["bond_requests.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("bond_request_id"),
    )
    op.create_index(
        op.f("ix_manifests_bond_request_id"),
        "manifests",
        ["bond_request_id"],
        unique=True,
    )
    op.create_index(
        op.f("ix_manifests_document_hash"), "manifests", ["document_hash"], unique=False
    )
